...
describe('Sendenv frontend', () => {
	...
	test('renders the existing homepage and header', () => {
		...
		const header = render(Header, { props: { apiBaseUrl } });

		expect(page.head).toContain('<title>Sendenv - Securely share secrets with anyone</title>');
		expect(page.body).toContain('Securely share secrets with anyone');
		expect(page.body).toContain('Generate link');
		expect(page.body).toContain('disabled');
		expect(header.body).toContain('width="127" height="30"');
		expect(header.body).toContain('API docs');
	});
	...
	test('renders an available secret without consuming it', () => {
		...
		expect(page.body).toContain('Someone shared a secret with you');
		expect(page.body).toContain('id="secret-output"');
		expect(page.body).toContain('Reveal secret');
		expect(page.body).toContain('Copy secret');
	});
	...
	test('renders only the error and new-secret action when a secret expired', () => {
		...
		expect(page.body).toContain('Someone shared a secret with you');
		expect(page.body).toContain('This link has expired.');
		expect(page.body).toContain('Create a new secret');
		expect(page.body).not.toContain('id="secret-output"');
		expect(page.body).not.toMatch(/<button[^>]*>Reveal secret<\/button>/);
		expect(page.body).not.toMatch(/<button[^>]*>Copy secret<\/button>/);
	});
